    ...
    return (
      <div className="bg-white p-3 rounded-lg shadow-lg border border-gray-200">
        ...
        <p className="text-sm text-gray-600">
          지출: ${data.value.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
        </p>
        <p className="text-sm text-gray-600">
            ...
            <tbody className="divide-y divide-gray-50">
              {data.map((platform, index) => (
                <tr key={index} className="hover:bg-gray-50">
                  ...
                  <td className="py-2 text-right text-gray-700">
                    ${platform.spend.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}
                  </td>
                  <td className="py-2 text-right">
                    <span className={`font-medium ${
                      ...
                      {platform.leads > 0 ? `$${platform.cpl.toFixed(2)}` : '-'}
                    </span>
                  </td>
                </tr>
              ))}
            ...
            <div className="mt-3 p-2 bg-green-50 rounded-lg border border-green-100">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-green-800">
                  ...
                  {' '}효율 최고 (CPL ${bestPlatform.cpl.toFixed(2)})
                </span>
              </div>
            </div>
          )}
