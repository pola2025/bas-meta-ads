...
const worker = new Worker(
  ...
  async (job) => {
    ...
    try {
      ...
      // 4. Daily Aggregates 생성 ⭐ NEW
      await generateDailyAggregates(clientId, weekStart, weekEnd);

      // 5. Weekly Summary 생성
      await generateWeeklySummary(clientId, weekStart, weekEnd);

      // 6. 텔레그램 리포트 발송 - 자동 발송 비활성화 (수동 발송만 허용)
...
/**
 * Daily Aggregates 생성 함수 ⭐ NEW
 */
async function generateDailyAggregates(clientId, weekStart, weekEnd) {
  try {
    // weekStart ~ weekEnd 기간의 모든 날짜에 대해 집계
    ...
    if (error) {
      console.error('❌ Failed to generate daily aggregates:', error);
      // Daily aggregates 실패는 치명적이지 않음 (다음 작업 계속 진행)
      return;
    }

    const totalRows = data?.reduce((sum, row) => sum + (row.rows_affected || 0), 0) || 0;
    console.log(`📊 Daily aggregates generated: ${totalRows} rows for ${weekStart} ~ ${weekEnd}`);

  } catch (error) {
    console.error('❌ Daily aggregates error:', error.message);
    // 에러 발생 시에도 작업 계속 진행
  }
}
