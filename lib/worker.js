 ...
 * @returns {Promise<{valid: boolean, endDate: string|null}>}
 */
async function checkServicePeriod(clientId) {
  const { data: client, error } = await supabase
    .from('clients')
    .select('service_end_date, is_active')
    .eq('id', clientId)
    .single();
  ...
  if (!client.is_active) {
    return { valid: false, endDate: client.service_end_date };
  }

  if (!client.service_end_date) {
    return { valid: true, endDate: null }; // 종료일 미설정 = 무제한
  }

  const today = new Date().toISOString().split('T')[0];
  const isValid = client.service_end_date >= today;

  return { valid: isValid, endDate: client.service_end_date };
}
  ...
  async (job) => {
    ...
    try {
      ...
      if (!servicePeriod.valid) {
        console.log(`⏹️ Service expired for ${clientName} (End: ${servicePeriod.endDate})`);
        return {
          success: false,
          clientId,
          clientName,
      ...
      // 4. Daily Aggregates 생성 + 정합성 검증 ⭐ IMPROVED
      const syncResult = await syncAndVerify(clientId, weekStart, weekEnd);
      if (!syncResult.success) {
        console.warn(`⚠️ Data integrity issue: ${syncResult.error}`);
        // 정합성 문제가 있어도 작업은 계속 진행 (치명적 오류 아님)
      }

      // 5. Weekly Summary 생성
      await generateWeeklySummary(clientId, weekStart, weekEnd);

      // 6. 누적 통계 + 일별 스냅샷 업데이트 ⭐ Phase 1 추가
      await runDailyAggregation(clientId, weekEnd);

      // 7. 텔레그램 리포트 발송 - 자동 발송 비활성화 (수동 발송만 허용)
...
// generateDailyAggregates 함수는 data-integrity.js의 syncAndVerify로 대체됨

/**
 * 누적 통계 + 일별 스냅샷 업데이트 ⭐ Phase 1 추가
 * - ad_cumulative_stats: 전체 기간 누적 통계
 * - ad_daily_snapshot: 해당 날짜 스냅샷 (순위 변동 추적)
 */
async function runDailyAggregation(clientId, targetDate) {
  try {
    const { data, error } = await supabase.rpc('run_daily_aggregation', {
      p_client_id: clientId,
      p_date: targetDate
    });

    if (error) {
      // 함수가 없으면 에러 로그만 출력 (테이블 미생성 상태 대비)
      if (error.message.includes('function') && error.message.includes('does not exist')) {
        console.log('⚠️ run_daily_aggregation function not found - skipping (Phase 1 migration needed)');
        return;
      }
    ...
    console.log(`📈 Daily aggregation completed for ${targetDate}`);
  } catch (err) {
    // 집계 실패는 치명적이지 않음 - 경고만 출력하고 계속 진행
    console.warn(`⚠️ Daily aggregation warning: ${err.message}`);
  }
}
...
async function updateAuthStatus(clientId, status) {
  ...
  if (error) {
    ...
    if (status === 'auth_required' || status === 'token_expired') {
      await sendAuthStatusAlert(clientName, status);
    }
  }
}
 ...
 */
async function sendAuthStatusAlert(clientName, status) {
