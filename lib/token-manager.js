...
class TokenManager {
   ...
   *
   * ⭐ OAuth 하이브리드 지원:
   * - auth_type='manual': 기존 로직 그대로 (status 체크 없음)
   * - auth_type='oauth': status='active'인 경우에만 토큰 반환
   */
  async getAccessToken(clientId) {
    if (!clientId) return null;

    try {
      // 1. DB에서 클라이언트 정보 조회 (auth_type, status 포함)
      const { data: client, error: dbError } = await this.supabase
        .from('clients')
        .select('client_name, auth_type, status, encrypted_access_token, meta_access_token_id')
        .eq('id', clientId)
        .single();
      ...
      // 2. ⭐ OAuth 클라이언트 status 체크 (핵심 추가 로직)
      if (client?.auth_type === 'oauth' && client?.status !== 'active') {
        console.log(`⚠️ OAuth client ${client.client_name || clientId} is not active (status: ${client.status})`);
        console.log(`   → 데이터 수집 스킵 (승인 대기 또는 중지 상태)`);
        return null;  // pending, suspended, expired 상태면 토큰 반환하지 않음
      }

      // 3. AES 암호화 토큰이 있으면 복호화
      ...
      // 4. Fallback: Vault에서 조회 (기존 클라이언트 호환)
