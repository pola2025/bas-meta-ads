...
class TokenManager {
  ...
  async ensureValidToken(clientId) {
    ...
    if (!isExpired && !isExpiringSoon) {
      ...
      // Vault에서 Access Token 조회
      return await this.getAccessToken(client.meta_access_token_id);
    }
  ...
  async getAccessToken(tokenId) {
    ...
    try {
      // Supabase Vault RPC 호출
      const { data, error } = await this.supabase.rpc('vault_read_secret', {
        secret_id: tokenId
      });

      if (error) {
        console.error('Failed to read access token from Vault:', error);
        return null;
      }
