...
async function saveWeeklyReport({
  ...
  const reportData = {
    ...
    spend_change: prevStats.spend > 0 ? ((thisStats.spend - prevStats.spend) / prevStats.spend) * 100 : 0,
    leads_change: prevStats.leads > 0 ? ((thisStats.leads - prevStats.leads) / prevStats.leads) * 100 : 0,
    cpl_change: prevStats.cpl > 0 ? ((thisStats.cpl - prevStats.cpl) / prevStats.cpl) * 100 : 0,
    ctr_change: prevStats.ctr > 0 ? ((thisStats.ctr - prevStats.ctr) / prevStats.ctr) * 100 : 0,
